...
/**
 * Lấy version info mới nhất cho project configuration
 * Tối ưu với cache để tránh query liên tục
 */
async function getProjectVersionInfo(projectId, defaultConnection, cache = null) {
    try {
        // Kiểm tra cache nếu có và còn hiệu lực (< 5 phút)
        if (cache && cache.projectId === projectId && cache.lastUpdated &&
            (Date.now() - cache.lastUpdated) < 5 * 60 * 1000) {
            return cache;
        }

        // Setup models
        const FieldConfigurationModel = mongoose.models[FieldConfigurationCollectionName] ||
            defaultConnection.model(FieldConfigurationCollectionName, FieldConfigurationSchema);
        const ProjectThresholdModel = mongoose.models[ProjectThresholdCollectionName] ||
            defaultConnection.model(ProjectThresholdCollectionName, ProjectThresholdSchema);

        // Lấy field configuration version mới nhất với isActive = true
        const fieldConfig = await FieldConfigurationModel.findOne(
            { project_id: new mongoose.Types.ObjectId(projectId), isActive: true },
            { version: 1, updated_at: 1, fields: 1 },
            { sort: { version: -1 } }
        ).lean();

        // Lấy project threshold version mới nhất với isActive = true
        const projectThreshold = await ProjectThresholdModel.findOne(
            { projectId: new mongoose.Types.ObjectId(projectId), isActive: true },
            { version: 1, modifiedDate: 1 },
            { sort: { version: -1 } }
        ).lean();

        // Prepare fieldNotCount từ field config
        const fieldNotCount = fieldConfig?.fields
            ? fieldConfig.fields
                .filter(field => field.is_report_count === false)
                .map(field => field.field_name)
            : [];

        const versionInfo = {
            projectId,
            fieldConfigVersion: fieldConfig?.version || null,
            fieldConfigUpdatedAt: fieldConfig?.updated_at || null,
            projectThresholdVersion: projectThreshold?.version || null,
            projectThresholdUpdatedAt: projectThreshold?.modifiedDate || null,
            fieldNotCount,
            lastUpdated: Date.now(),
            // Flag để detect version changes
            configHash: `${fieldConfig?.version || 0}-${projectThreshold?.version || 0}`
        };

        loggerInfo.info(`[QC Cron Job] Retrieved version info for project ${projectId}: Field Config v${versionInfo.fieldConfigVersion}, Project Threshold v${versionInfo.projectThresholdVersion}`);

        return versionInfo;
    } catch (error) {
        loggerError.error(`[QC Cron Job] Error retrieving version info for project ${projectId}:`, error);

        // Fallback: return basic info to continue processing
        return {
            projectId,
            fieldConfigVersion: null,
            fieldConfigUpdatedAt: null,
            projectThresholdVersion: null,
            projectThresholdUpdatedAt: null,
            fieldNotCount: [],
            lastUpdated: Date.now(),
            configHash: '0-0',
            hasError: true
...
/**
 * Kiểm tra xem version có thay đổi so với cache hiện tại không
 */
function hasVersionChanged(newVersionInfo, cachedVersionInfo) {
    if (!cachedVersionInfo) return true;

    return newVersionInfo.configHash !== cachedVersionInfo.configHash;
}
...
async function prepareDocumentSteps({
    ...
    versionInfo = null // Thêm version info để gắn vào metadata
}) {
    ...
    if (mistake_details.length > 0) {
        ...
        if (!doc.batch_id) {
            ...
            try {
                ...
                // Chuẩn bị version info để gắn vào metadata
                const mistakeVersionUpdate = {
                ...
                // Gắn version info nếu có
                if (versionInfo) {
                    mistakeVersionUpdate.field_configuration_version = versionInfo.fieldConfigVersion;
                    mistakeVersionUpdate.project_threshold_version = versionInfo.projectThresholdVersion;
                    mistakeVersionUpdate.processing_version_info = {
                        field_config_updated_at: versionInfo.fieldConfigUpdatedAt,
                        project_threshold_updated_at: versionInfo.projectThresholdUpdatedAt,
    ...
    if (allQCEfforts.length > 0) {
        ...
        // Gắn version info nếu có
        if (versionInfo) {
            effortDoc.field_configuration_version = versionInfo.fieldConfigVersion;
            effortDoc.project_threshold_version = versionInfo.projectThresholdVersion;
            effortDoc.processing_version_info = {
                field_config_updated_at: versionInfo.fieldConfigUpdatedAt,
                project_threshold_updated_at: versionInfo.projectThresholdUpdatedAt,
                processed_at: new Date()
            };
...
async function handleDocumentComplete(args) {
    const { doc, projectId, fieldNotCount, filterControl, sectionMultiRow, DocumentHistoryModel, versionInfo } = args;
    ...
    const { bulkOpsMistake, bulkOpsEffort, insertedMistake, insertedEffort, newCompletedSteps, processed } = await prepareDocumentSteps({
        doc, projectId, fieldNotCount, filterControl, sectionMultiRow, DocumentHistoryModel, versionInfo
    });
...
async function processQCData() {
    ...
    try {
        ...
        for (const project of projects) {
            ...
            // NEW: Lấy version info cho project và cache trong quá trình xử lý
            let projectVersionCache = await getProjectVersionInfo(projectId, defaultConnection);

            // Legacy fallback: nếu không có version info, vẫn lấy field config theo cách cũ
            let fieldNotCount = projectVersionCache.fieldNotCount;
            if (fieldNotCount.length === 0) {
                loggerInfo.warn(`[QC Cron Job] No field configuration found via version system for project ${projectId}, falling back to legacy method`);
                const fieldsNotCounted = await FieldConfigurationModel.find(
                    { is_report_count: false, project_id: project._id },
                    { field_name: 1, _id: 0 }
                ).lean();
                fieldNotCount = fieldsNotCounted.map(field => field.field_name);
            }
            ...
            // Version change detection - check mỗi 100 documents hoặc 5 phút
            let docProcessedCount = 0;
            const VERSION_CHECK_INTERVAL = 100; // documents
            const VERSION_CHECK_TIME_INTERVAL = 5 * 60 * 1000; // 5 phút
            ...
            for (const batch of batches) {
                ...
                if (batch.status >= 100) {
                    ...
                    for await (const doc of docCursor) {
                        ...
                        // Kiểm tra version changes định kỳ để refresh cache nếu cần
                        if (docProcessedCount % VERSION_CHECK_INTERVAL === 0 ||
                            (projectVersionCache.lastUpdated && Date.now() - projectVersionCache.lastUpdated > VERSION_CHECK_TIME_INTERVAL)) {

                            const newVersionInfo = await getProjectVersionInfo(projectId, defaultConnection, null); // Force refresh cache

                            if (hasVersionChanged(newVersionInfo, projectVersionCache)) {
                                loggerInfo.info(`[QC Cron Job] Version change detected for project ${projectId}: ${projectVersionCache.configHash} -> ${newVersionInfo.configHash}`);
                                projectVersionCache = newVersionInfo;
                                fieldNotCount = newVersionInfo.fieldNotCount;

                                // Log version change để audit
                                loggerInfo.info(`[QC Cron Job] Updated to Field Config v${newVersionInfo.fieldConfigVersion}, Project Threshold v${newVersionInfo.projectThresholdVersion} for project ${projectId}`);
                            }
                        }
                        ...
                        if (doc.status >= 450) {
                            ...
                            result = await handleDocumentComplete({
                                ...
                                sectionMultiRow,
                                DocumentHistoryModel,
                                versionInfo: projectVersionCache
                            });
            ...
            // Log version usage summary for project
            loggerInfo.info(`[QC Cron Job] Completed project ${project.projectName} (${projectId}) processing with versions:`, {
                fieldConfigVersion: projectVersionCache.fieldConfigVersion,
                projectThresholdVersion: projectVersionCache.projectThresholdVersion,
                documentsProcessed: docProcessedCount,
                finalConfigHash: projectVersionCache.configHash,
                versionCacheRefreshes: projectVersionCache.lastUpdated ? Math.floor((Date.now() - processStartTime) / VERSION_CHECK_TIME_INTERVAL) : 0
            });
        }
